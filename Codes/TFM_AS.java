	...
	public static double[][] TCA(double[][] dem, double cellsize){
		...
		while(priQ.size()!=0){
			...
			for(int t=1;t<9;t++){
				...
				if(r[t]<pi4+att){
					//cell[t]=1.0;
					if(z2>z1-att && z3<z1-att){
						diagonal[t]=1.0;
					}else if(z2<z1-att && z3>z1-att){
						cardinal[t]=1.0;
					}else{
						diagonal[t]=s2/(s1+s2);
						cardinal[t]=s1/(s1+s2);
					...
					if(r[pp1]<pi4+att || r[pp1]>pi45-att){
						...
						if(z2>z1-att && z3<z1-att){
							diagonal[t]=1.0-facet[t][0];
						}else if(z2<z1-att && z3>z1-att){
							cardinal[t]=1.0-facet[t][0];
						}else{
							diagonal[t]=(1.0-facet[t][0])*s2/(s1+s2);
							cardinal[t]=(1.0-facet[t][0])*s1/(s1+s2);
						}
						hasUp[pp1]++;
					}else{
					...
					if(r[pp1]<pi4+att || r[pp1]>pi45-att){
						...
						center[t]=center[t]+1.0-Math.tan(r1);
					...
					if(r[pp2]<pi+att){
						...
						//facet[t][1]=Math.tan(r1)/(1.0+Math.tan(r1));
						//facet[t][1]=Math.sin(r1)/(Math.sin(r1)+Math.cos(r1));
						hasUp[pp2]++;
					}else{
						//cardinal[t]=Math.tan(r1)/(1.0+Math.tan(r1));
						//cardinal[t]=Math.tan(r1)/(1.0+Math.tan(r1));
						if(z2>z1-att && z3<z1-att){
							diagonal[t]=diagonal[t]+Math.tan(r1)/(1.0+Math.tan(r1));
							//diagonal[t]=diagonal[t]+Math.sin(r1)/(Math.sin(r1)+Math.cos(r1));
						}else if(z2<z1-att && z3>z1-att){
							cardinal[t]=cardinal[t]+Math.tan(r1)/(1.0+Math.tan(r1));
							//cardinal[t]=cardinal[t]+Math.sin(r1)/(Math.sin(r1)+Math.cos(r1));
						}else{
							diagonal[t]=diagonal[t]+(Math.tan(r1)/(1.0+Math.tan(r1)))*s2/(s1+s2);
							//cardinal[t]=cardinal[t]+(Math.tan(r1)/(1.0+Math.tan(r1)))*s1/(s1+s2);
							diagonal[t]=diagonal[t]+(Math.sin(r1)/(Math.sin(r1)+Math.cos(r1)))*s2/(s1+s2);
							//cardinal[t]=cardinal[t]+(Math.sin(r1)/(Math.sin(r1)+Math.cos(r1)))*s1/(s1+s2);
						}
					}
					//cell[t]=1.0-Math.tan(r1)/(1.0+Math.tan(r1));
					if(z2>z1-att && z3<z1-att){
						diagonal[t]=diagonal[t]+1.0-Math.tan(r1)/(1.0+Math.tan(r1));
						//diagonal[t]=diagonal[t]+1.0-Math.sin(r1)/(Math.sin(r1)+Math.cos(r1));
					}else if(z2<z1-att && z3>z1-att){
						cardinal[t]=cardinal[t]+1.0-Math.tan(r1)/(1.0+Math.tan(r1));
						//cardinal[t]=cardinal[t]+1.0-Math.sin(r1)/(Math.sin(r1)+Math.cos(r1));
					}else{
						diagonal[t]=diagonal[t]+(1.0-Math.tan(r1)/(1.0+Math.tan(r1)))*s2/(s1+s2);
						cardinal[t]=cardinal[t]+(1.0-Math.tan(r1)/(1.0+Math.tan(r1)))*s1/(s1+s2);
						//diagonal[t]=diagonal[t]+(1.0-Math.sin(r1)/(Math.sin(r1)+Math.cos(r1)))*s2/(s1+s2);
						//cardinal[t]=cardinal[t]+(1.0-Math.sin(r1)/(Math.sin(r1)+Math.cos(r1)))*s1/(s1+s2);
					}
				}
			}
			...
			while(num>0 && border==false){
				...
				for(int t=1;t<9;t++){
					if(hasUp[t]==0 && treat[t]==false){
						...
						if(cardinal[t]>att || diagonal[t]>att){
							//total_out=total_out+p[t]*(cell[t]+cardinal[t]+diagonal[t]);
							//facet_out[t]=p[t]*(cell[t]+cardinal[t]+diagonal[t]);
							total_out=total_out+p[t]*(cardinal[t]+diagonal[t]);
							facet_out[t]=p[t]*(cardinal[t]+diagonal[t]);
						}else{
							facet_out[t]=0.0;
						}
					}
			...
			if(center_flow>att && border==false){
				...
				for(int t=1;t<9;t++){
					if(facet_out[t]>att){
						...
						//}
						if(cardinal[t]>att){
							tca[xx-1+a[pp0]][yy-1+b[pp0]]=tca[xx-1+a[pp0]][yy-1+b[pp0]]+(center_flow*facet_out[t]/total_out)*(cardinal[t]/(cardinal[t]+diagonal[t]));
							amount[xx+a[pp0]][yy+b[pp0]][0]=amount[xx+a[pp0]][yy+b[pp0]][0]+(center_flow*facet_out[t]/total_out)*(cardinal[t]/(cardinal[t]+diagonal[t]));
						}
						if(diagonal[t]>att){
							tca[xx-1+a[pp1]][yy-1+b[pp1]]=tca[xx-1+a[pp1]][yy-1+b[pp1]]+(center_flow*facet_out[t]/total_out)*(diagonal[t]/(cardinal[t]+diagonal[t]));
							amount[xx+a[pp1]][yy+b[pp1]][0]=amount[xx+a[pp1]][yy+b[pp1]][0]+(center_flow*facet_out[t]/total_out)*(diagonal[t]/(cardinal[t]+diagonal[t]));
						}
					}
				}
			}
